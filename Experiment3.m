...
counter=1;
for loop=6:2:10
N_1=2*(loop^2)+1%round(sqrt(N)./2).*2+1; % number of points in x_1-1
...
%time(counter)=toc;
% steps(counter)=st;
  points(counter)=N_1*N_2;
% error=sum(sum((A_0-correct_solution)).^2);
% relerror(counter)=error/(sum(sum(correct_solution)));
  counter=counter+1;
end %% Plot A function
...
figure 
 ...
 plot(points,S3(:,1),'black')
 plot(points,S3(:,2),'--black')
...
%% Plot times
 ...
 plot(points,T2(:,1),'b')
 plot(points,T2(:,2),'--b')
 plot(points,T3(:,1),'black')
 plot(points,T3(:,2),'--black')
 legend('M_0,E_1','M_0,E_2','M_1,E_1','M_1,E_2','M_2,E_1','M_2,E_2')
 %% Plot A function
